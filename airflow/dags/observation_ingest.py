...
def _fetch_range(start_d: date, end_d: date) -> pd.DataFrame:
    log.info("fetch_range: %s -> %s (UTC), lat=%.4f lon=%.4f city=%s",
             start_d, end_d, LAT, LON, CITY)
    loc = Point(LAT, LON)
    ...
    if df.empty:
        return pd.DataFrame(columns=["valid_time","temperature_2m","precipitation","wind_speed_10m","city"])
...
def _upsert(df: pd.DataFrame):
    ...
    """, list(df[["city","valid_time","temperature_2m","precipitation","wind_speed_10m"]]
...
with DAG(
    ...
    t1 = PythonOperator(task_id="load_initial_history", python_callable=load_initial_history)
